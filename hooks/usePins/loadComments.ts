...
import { PinProps, CommentReactionPin } from '@/types';
...
export const loadComments = async (pageId: string, setPins: (pins: PinProps[]) => void, setComments: (comments: { [key: string]: string }) => void, session: Session | null): Promise<void> => {
    ...
    try {
        const { data: commentsData, error: commentsError } = await supabase.from('comments').select('*').eq('page_id', pageId);
        ...
        const pinsData = commentsData.map(comment => {
            ...
            return {
                id: comment.id, x: comment.pos_x, y: comment.pos_y, num: comment.pin_number, comment: comment.content || '', created_at: comment.created_at, status: comment.status || 'ativo', user_id: comment.user_id, page_id: comment.page_id, reactions: reactions, url_comentario: comment.url_comentario || null, iframeSize: comment.iframe_size || null
            };
        });
        ...
        const processedPins = pinsData.map(pin => ({
            ...
            reactions: pin.reactions?.map((reaction: CommentReactionPin) => ({
                ...reaction,
