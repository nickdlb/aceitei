...
export default function BlogPageClient({ posts }: { posts: Post[] }) {
  ...
  return (
    <main className="w-full max-w-[1400px] mx-auto px-4 py-12">
      ...
      <input type="text" placeholder="Buscar por título ou resumo..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full max-w-md border border-gray-300 rounded-lg px-4 py-2 mb-10 focus:outline-none focus:ring-2 focus:ring-acazul" />
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {filteredPosts.map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group block bg-gray-50 border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition" >
            {post.metadata.coverImage && (
              <div className="relative h-48 w-full">
                <Image src={post.metadata.coverImage} alt={`Imagem de capa: ${post.metadata.title}`} layout="fill" objectFit="cover" className="group-hover:scale-105 transition-transform duration-300" />
              </div>
            )}
            <div className="p-5">
