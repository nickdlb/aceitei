...
export const uploadAndInsertFile = async (file: File, onUploadProgress?: (progress: number) => void) => {
    try {
                    ...
                    const { data: insertData, error: insertError } = await supabase.from('images').insert([
                        {
                            ...
                            image_url: url,
                            user_id: 'a08255eb-5731-422e-80e6-80c317d4fcb1', // Replace with actual user ID
                        },
                    ]).select();
