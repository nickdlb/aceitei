...
const CommentBar = ({
  ...
  const [permissions, setPermissions] = useState<{
    [key: string]: { canEdit: boolean; canDelete: boolean; canChangeStatus: boolean };
  }>({});
  const [replyText, setReplyText] = useState('');
  const [showReplies, setShowReplies] = useState<{ [key: string]: boolean }>({});
  const openRepliesRef = useRef<{ [key: string]: boolean }>({});
  const [userNames, setUserNames] = useState<{ [key: string]: string }>({});
  const [expandedUrls, setExpandedUrls] = useState<{ [url: string]: boolean }>({});
  const [groupByFilter, setGroupByFilter] = useState<'Link' | 'Nenhum'>('Nenhum');
  ...
  const CommentDelete = async (pinId: string) => {
    try {
      await deleteComment(pinId, pins, () => loadComments(), () => loadComments(), setEditingPinId, setRefreshKey, session);
      await loadComments();
    } catch (error) {
  ...
  const CommentStatusChange = async (pinId: string) => {
    await changeCommentStatus(pinId, pins, () => loadComments(), session, loadComments);
  };
  ...
  const groupedPins = pins.reduce((acc, pin) => {
    const url = pin.url_comentario || 'Sem URL';
    if (!acc[url]) acc[url] = [];
    acc[url].push(pin);
    return acc;
  }, {} as { [url: string]: PinProps[] });
